...
import { selectSetUser } from '../../features/auth';
import { useDispatch, useSelector } from 'react-redux';

import { createToken, createSessionId, tmdbAuth } from '../../utils/auth';
    ...
    const logIn = async() => {
        if(token){
          if(sessionId){
            ...
            const {data:  userData} = await tmdbAuth.get(`/account/null?session_id=${newSession}`);
    ...
    <>
      <AppBar position='fixed'>
        ...
        {isMobile && <IconButton
          ...
          {!isMobile && <Search />}
          {!isAuthenticated ? 
          <Button color='inherit' onClick={createToken}>
            Login &nbsp; <AccountCircle />
          </Button> : 
